      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          vector<double> mpc_x_vals(N-1);
          vector<double> mpc_y_vals(N-1);

          for (int i=0; i<N-1; i++)
          {
            mpc_x_vals[i] = vars[2+i];
            mpc_y_vals[i] = vars[2+i+N-1];
          }
          ...
          vector<double> next_x_vals(ptsx_eigen.size());
          vector<double> next_y_vals(ptsx_eigen.size());

          //.. add (x,y) points to list here, points are in reference to the vehicle's coordinate system
          // the points in the simulator are connected by a Yellow line
          for (int i=0; i<ptsx_eigen.size(); i++)
          {
